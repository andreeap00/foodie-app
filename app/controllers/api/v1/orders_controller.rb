class Api::V1::OrdersController < Api::V1::ApplicationController
  ...
  def purchase
    order = current_user.orders.find(params[:id])
    render json: { error: "Not Found Order" }, status: :not_found if order.nil? 
  ...
  def remove_from_order
    ...
    render json: { error: "#{product.title} not found in the cart." }, status: :unprocessable_entity if !order_product.present?
